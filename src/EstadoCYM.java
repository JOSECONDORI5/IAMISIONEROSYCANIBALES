 ...
 *
 * @author Jose Condori
 * @version 2 jun. 2021 11:28:57
 */
public class EstadoCYM implements Estado {
    ...
    private void transportarYGuardar(int m, int c, ArrayList<Estado> suc) {
        ...
        suc.add(new EstadoCYM(boteCopia));
    }
    ...
    public ArrayList<Estado> generarSucesores() {
        ...
        if (boteActual[2] == 1) {
            ...
            if (boteActual[0] >= 1 && complemento[0] + 1 >= complemento[1] && (boteActual[0] - 1 >= boteActual[1] || boteActual[0] == 1)) {
                transportarYGuardar(-1, 0, sucesores);
            }
            //2) 2 misioneros cruzan en el bote
            if (boteActual[0] >= 2 && (boteActual[0] - 2 >= boteActual[1] || boteActual[0] == 2) && complemento[0] + 2 >= complemento[1]) {
        ...
        if (boteActual[2] == 0) {
            //1) 1 misionero cruza en el bote
            if (complemento[0] >= 1 && boteActual[0] + 1 >= boteActual[1] && (complemento[0] - 1 >= complemento[1] || complemento[0] == 1)) {
                transportarYGuardar(1, 0, sucesores);
            }
            //2) 2 misioneros cruzan en el bote
            if (complemento[0] >= 2 && (complemento[0] - 2 >= complemento[1] || complemento[0] == 2) && boteActual[0] + 2 <= boteActual[1]) {
                transportarYGuardar(2, 0, sucesores);
            }
            //3) 1 canibal cruza en el bote
    ...
    public boolean igual(Estado e) {
        if (Arrays.equals(boteActual, ((EstadoCYM) e).getBoteActual())) {
            return true;
        }
        return false;
    }
